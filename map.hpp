...
void randomMapGenerate(char el, int count) {//рандомно расставл€ем камни
	...
	while (count > 0) {
		...
		if (TileMap[randomElementY][randomElementX] == ' ') {//если встретили символ пробел, 
			TileMap[randomElementY][randomElementX] = el; //то ставим туда камень.
			//std::cout << "coordinate of Stone X:" << randomElementX << "\n" << "coordinate of Stone Y:" << randomElementY << "\n\n";
			count--;//создали камень=>счетчик камней будет "текущий минус 1"
		}
	}
}